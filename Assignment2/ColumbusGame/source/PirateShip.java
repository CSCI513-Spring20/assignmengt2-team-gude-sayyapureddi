...
//        each pirate island launches one pirate ship and Observer class is added
public class PirateShip implements Observer {
	private Point currentLocation;
	int[][] ocean;

	// constructor Pirate ship
...
//    its then used checked with the best possible next step (Greedy) from the possible for the pirate ship
	public void update(Observable o, Object arg) {
		...
		// using Manhattan distance for the distance between the pirate ship and
		// columbus ship
        ...
        //  checking left
			...
			if (temp != 0) {
				if (temp < distance && ocean[currentLocation.x - 1][currentLocation.y] != 2
						&& ocean[currentLocation.x - 1][currentLocation.y] != 3
						&& ocean[currentLocation.x - 1][currentLocation.y] != 4) {
					distance = temp;
       ...
       //  checking right
		...
		if (currentLocation.y < 9 && flag == 1) {
			...
			if (temp != 0) {
				if (temp < distance && ocean[currentLocation.x][currentLocation.y + 1] != 2
						&& ocean[currentLocation.x][currentLocation.y + 1] != 3
		...
		int temp = Math.abs(currentLocation.x - shipLocation.x) + Math.abs(currentLocation.y - shipLocation.y);
		if (temp == 1) {
			OceanExplorer.text.setText("Pirate Ship Caught Your Ship!");
			OceanExplorer.text2.setText("Click Reset to Play Again");
			OceanExplorer.label.setText("Pirate Ship Caught Your Ship!,Click Reset to Play Again");
			// System.out.println("Pirate Ship Caught Your Ship");
			OceanExplorer.close = true;
		}
